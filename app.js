...
function renderDashboard(){
 ...
 $('workspaceSummary').textContent=state.profile.businessName?`${state.profile.businessName} has ${state.evidence.length} reusable evidence item${state.evidence.length===1?'':'s'} and ${state.requests.length} saved customer response${state.requests.length===1?'':'s'}.`:'Your sample workspace is saved automatically in this browser.';
}
...
function renderEvidence(){
 ...
 if(!state.evidence.length){$('evidenceList').innerHTML='<div class="empty">No evidence yet. Add a sample item or load the realistic starter pack.</div>';return;}
...
function autoMatch(text){
 ...
 if(!candidates.length)return{status:'Missing',evidenceId:null,reason:'No likely evidence found in this sample library.'};
 const e=candidates[0].e;if(isExpired(e))return{status:'Expired',evidenceId:e.id,reason:'Likely evidence exists, but its review/expiry date has passed.'};if(e.review==='Needs review'||candidates[0].score<4)return{status:'Needs review',evidenceId:e.id,reason:'Possible supporting evidence exists and should be checked by a person.'};return{status:'Matched',evidenceId:e.id,reason:'Likely demo match based on public sample keywords. Human confirmation is still required.'};
}
...
function renderResults(){
 const r=state.activeRequest;if(!r){$('resultsList').innerHTML='<div class="empty">No active customer request. Create one in step 3 or open a saved request.</div>';$('resultScore').textContent='No request';$('resultIntro').textContent='Run a customer request to see results.';$('responseNotes').value='';return;}
 const ready=r.requirements.filter(x=>x.status==='Matched').length;$('resultIntro').textContent=`${r.buyer}: ${r.requirements.length} requirements checked against ${state.evidence.length} reusable evidence items.`;$('resultScore').textContent=`${ready}/${r.requirements.length} ready`;$('responseNotes').value=r.notes||'';
 ...
 document.querySelectorAll('[data-status]').forEach(s=>s.onchange=()=>{const x=r.requirements.find(q=>q.id===s.dataset.status);x.status=s.value;x.reason='Human-reviewed decision in this prototype.';save();renderResults();});
...
function renderHistory(){
 ...
 if(!state.requests.length){$('requestHistory').innerHTML='<div class="empty">No saved customer responses yet. Complete steps 1–4, save the response, then return here.</div>';return;}
