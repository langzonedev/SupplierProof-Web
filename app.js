...
function renderEvidence(){
 ...
 $('evidenceList').innerHTML=state.evidence.map(e=>{const [health,cls]=evidenceHealth(e);const attachment=e.fileName?`<div class="attachment-row"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M7 3h7l4 4v14H7z" fill="none" stroke="currentColor" stroke-width="1.8"/><path d="M14 3v5h5" fill="none" stroke="currentColor" stroke-width="1.8"/></svg><span>${esc(e.fileName)}</span><small>${esc(e.fileSizeLabel||'')}</small></div>`:'';return `<article class="evidence-item"><div class="row"><div><strong>${esc(e.type)}</strong><div class="meta">${esc(e.ref||'No reference')}</div></div><span class="pill ${cls}">${health}</span></div><div class="meta">Owner: ${esc(e.owner||'Unassigned')} · Issued ${e.issueDate||'—'} · Review/expiry ${e.expiryDate||'not set'}</div>${attachment}${e.notes?`<div>${esc(e.notes)}</div>`:''}<div class="item-actions">${e.fileKey?`<button class="mini" data-download-file="${e.id}">Download file</button>`:''}<button class="mini" data-review-evidence="${e.id}">Toggle review</button><button class="mini danger" data-remove-evidence="${e.id}">Remove</button></div></article>`}).join('');
 ...
 document.querySelectorAll('[data-remove-evidence]').forEach(b=>b.onclick=()=>{window.SupplyfolioAttachments?.remove(b.dataset.removeEvidence);state.evidence=state.evidence.filter(x=>x.id!==b.dataset.removeEvidence);save();renderEvidence();renderResults();});
}
...
function summary(){
 const r=state.activeRequest;if(!r)return'No Supplyfolio customer request has been mapped yet.';
 return [`Supplyfolio customer checklist`,`Supplier: ${state.profile.businessName||'Not supplied'}`,`Customer: ${r.buyer}`,`Reference: ${r.ref||'Not supplied'}`,`Due: ${r.dueDate||'Not supplied'}`,`Channel: ${r.portal||'Not supplied'}`,'','Important: review every item before sending. Supplyfolio does not certify compliance.','',...r.requirements.map((x,i)=>{const e=getEvidence(x.evidenceId);return `${i+1}. [${x.status}] ${x.text}${e?`\n   Evidence: ${e.type} — ${e.ref||'No reference'}${e.fileName?`\n   File: ${e.fileName}`:''}`:''}\n   ${x.reason}`;}),'',`Internal notes: ${r.notes||'None'}`].join('\n');
}
...
$('downloadSummary').addEventListener('click',()=>{const blob=new Blob([summary()],{type:'text/plain'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`supplyfolio-${(state.activeRequest?.buyer||'request').toLowerCase().replace(/[^a-z0-9]+/g,'-')}.txt`;a.click();setTimeout(()=>URL.revokeObjectURL(a.href),500);});
...
$('resetDemo').addEventListener('click',async()=>{if(confirm('Reset all Supplyfolio sample data saved in this browser?')){await window.SupplyfolioAttachments?.clear();localStorage.removeItem(STORE_KEY);location.reload();}});
